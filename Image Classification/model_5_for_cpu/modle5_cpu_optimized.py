...
def train_model_with_scheduler(model, trainloader, testloader, device, epochs=10):
    ...
    criterion = nn.CrossEntropyLoss(label_smoothing=0.2)  # 增加標籤平滑防過擬合 (0.15->0.2)
    optimizer = optim.AdamW(
        model.parameters(),
        lr=0.0006,  # 降低基礎學習率適應長訓練 (0.0008->0.0006)
        weight_decay=0.025,  # 大幅增加權重衰減防止過擬合 (0.015->0.025)
        betas=(0.9, 0.999),
        eps=1e-8,
    )
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=0.0015,  # 降低最大學習率適應長訓練 (0.002->0.0015)
        epochs=epochs,
        steps_per_epoch=len(trainloader),
        pct_start=0.15,  # 減少升溫時間比例 (0.2->0.15)
        anneal_strategy="cos",
        final_div_factor=50,  # 增加最終學習率衰減 (20->50)
    )
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            torch.nn.utils.clip_grad_norm_(
                model.parameters(), max_norm=0.3  # 更嚴格的梯度裁剪防過擬合 (0.5->0.3)
            )
            optimizer.step()
        ...
        if current_overfitting > overfitting_threshold:
            ...
            print(f"   ⚠️  過擬合警告: 差異 {current_overfitting:.2f}% (連續 {consecutive_overfitting} 次)")
