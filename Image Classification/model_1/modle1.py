...
def load_cifar10_data_enhanced(quick_test=True):
    ...
    if quick_test:
        trainset = Subset(trainset, range(5000))  # 增加到5000個樣本
        testset = Subset(testset, range(1000))  # 增加到1000個樣本
...
def evaluate_model_with_visualization(model, testloader, device, classes):
    ...
    plt.title("各類別準確率", fontsize=14, fontweight="bold")
    plt.ylabel("準確率 (%)")
    ...
    plt.title("標準化混淆矩陣", fontsize=14, fontweight="bold")
    plt.xlabel("預測類別")
    plt.ylabel("真實類別")
    ...
    plt.title("測試集類別分佈", fontsize=14, fontweight="bold")
...
def plot_training_history(train_losses, train_accs, val_accs):
    ...
    # 損失曲線
    plt.subplot(1, 3, 1)
    plt.plot(train_losses, "b-", linewidth=2, label="訓練損失")
    plt.title("訓練損失曲線", fontsize=14, fontweight="bold")
    plt.xlabel("Epoch")
    plt.ylabel("損失值")
    ...
    # 準確率曲線
    plt.subplot(1, 3, 2)
    plt.plot(train_accs, "g-", linewidth=2, label="訓練準確率")
    plt.plot(val_accs, "r-", linewidth=2, label="驗證準確率")
    plt.title("準確率曲線", fontsize=14, fontweight="bold")
    plt.xlabel("Epoch")
    plt.ylabel("準確率 (%)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    ...
    plt.plot(diff, "purple", linewidth=2, label="訓練-驗證差異")
...
def visualize_sample_predictions(model, testloader, device, classes, num_samples=12):
    ...
    fig.suptitle("gMLP 預測結果示例", fontsize=16, fontweight="bold")
...
def main():
    ...
    try:
        ...
        train_losses, train_accs, val_accs = train_model_with_scheduler(
            model, trainloader, testloader, device, epochs=30
        )
        ...
        # 7. 保存模型
        torch.save(model.state_dict(), "best_gmlp_model.pth")
        print("\n💾 模型已保存為 'best_gmlp_model.pth'")
