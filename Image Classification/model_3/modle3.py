...
def load_cifar10_data_enhanced(quick_test=True):
    ...
    transform_train = transforms.Compose(
        [
            ...
            transforms.ColorJitter(
                brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1  # 增強顏色變化
            ),
            ...
            transforms.RandomErasing(p=0.2, scale=(0.02, 0.15)),  # 隨機擦除
        ]
    )
    ...
    if quick_test:
        trainset = Subset(trainset, range(2000))  # 增加訓練樣本到2000
        testset = Subset(testset, range(500))  # 增加測試樣本到500
...
def create_optimized_gmlp_model():
    ...
    model = gMLPVision(
        image_size=32,
        patch_size=4,  # 減小patch size獲得更細致的特徵
        num_classes=10,
        dim=384,  # 增加維度提升表達能力
        depth=8,  # 增加深度但不會太深
        ff_mult=4,  # 前饋網絡倍數
...
def train_model_with_scheduler(model, trainloader, testloader, device, epochs=10):
    ...
    optimizer = optim.AdamW(
        model.parameters(), lr=0.0005, weight_decay=0.02
    )  # 降低學習率，增加權重衰減
    ...
    try:
        ...
        train_losses, train_accs, val_accs, epoch_times, total_training_time = (
            train_model_with_scheduler(
                model, trainloader, testloader, device, epochs=15
            )  # 增加epochs
        )
