...
def load_cifar10_data_enhanced(quick_test=True):
    ...
    if quick_test:
        # CPU防過擬合優化：大幅增加數據量減少過擬合
        trainset = Subset(trainset, range(5000))  # 大幅增加至5K樣本防過擬合 (3K->5K)
        testset = Subset(testset, range(1000))  # 相應增加測試數據至1K (600->1K)
        print("   🛡️ CPU防過擬合模式：增加數據量提升泛化能力")

    # CPU防過擬合DataLoader優化 - 增加數據量
    batch_size = 64  # 增加批次大小配合更多數據 (32->64)
    ...
    print(f"   ✓ CPU防過擬合優化: batch_size={batch_size}, 大數據量防過擬合")
