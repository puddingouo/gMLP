...
def create_optimized_gmlp_model(model_size="Ti"):
    """創建論文標準的 gMLP 模型架構"""
...
def train_model_with_scheduler(model, trainloader, testloader, device, epochs=300):
    """論文標準訓練配置 - 基於ImageNet-1K超參數 + 過擬合早停"""
    ...
    # 過擬合早停配置
    overfitting_patience = 15  # 過擬合檢測耐心值
    overfitting_counter = 0
    overfitting_threshold = 12.0  # 訓練-驗證差異閾值
    min_epochs_before_overfitting_check = 20  # 最少訓練epochs後才開始檢測過擬合

    # 論文不使用動態過擬合檢測，專注於標準訓練
    print("   📄 使用論文標準訓練配置：300 epochs + 標準正則化 + 過擬合早停")
    ...
    if len(train_losses) < epochs:
        final_train_val_diff = train_accs[-1] - val_accs[-1]
        if overfitting_counter >= overfitting_patience:
    ...
    try:
        ...
        train_losses, train_accs, val_accs, epoch_times, total_training_time = (
            train_model_with_scheduler(
                ...
                testloader,
                device,
                epochs=100,  # 論文標準300 epochs，小規模100
            )
        )
