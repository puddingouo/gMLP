...
class gMLPBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.proj_in = nn.Sequential(nn.Linear(dim, dim_ff), nn.ReLU())

        self.attn = (
            Attention(dim, dim_ff // 2, attn_dim, causal) if exists(attn_dim) else None
        )
...
class gMLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_logits = (
            nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, num_tokens))
